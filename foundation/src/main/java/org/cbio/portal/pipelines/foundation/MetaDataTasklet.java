...
public class MetaDataTasklet implements Tasklet {
    ...
    /**
     * Load/Modify gene panel meta data and write to meta file.
     * @param executionContext
     * @throws Exception
     */
    private void writeGenePanelMetaFile(ExecutionContext executionContext) throws Exception {
        String metaFilename = outputDirectory + "meta_gene_matrix.txt";

        List<String> genePanelData = new ArrayList();
        for (String key : genePanelMetaData.keySet()) {
            String value = genePanelMetaData.get(key).replace("<study_id>", cancerStudyIdentifier);
            genePanelData.add(key+":"+value);
        }
